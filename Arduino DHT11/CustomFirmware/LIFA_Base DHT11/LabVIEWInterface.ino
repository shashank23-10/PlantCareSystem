  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    *********************************************************************************/
    case 0xFE:
      ...
      dht.begin(command[2], command[3]); // DHT_Pin, DHT_Type
      unsigned long dht_data;
      dht_data = dht.readRawData();
      Serial.write(dht_data & 0xFF);
      Serial.write((dht_data >> 8) & 0xFF);
      Serial.write((dht_data >> 16) & 0xFF);
      Serial.write(dht_data >> 24);
      break;
